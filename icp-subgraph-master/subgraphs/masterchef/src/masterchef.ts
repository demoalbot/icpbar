...
function getMasterChef(block: ethereum.Block): MasterChef {
  ...
  if (masterChef === null) {
    ...
    masterChef.rewardPerBlock = contract.rewardPerBlock()
...
export function getPool(id: BigInt, block: ethereum.Block): Pool {
  ...
  if (pool === null) {
    ...
    pool.accRewardPerShare  = poolInfo.value3
...
export function logPoolAddition(event: LogPoolAddition): void {
  ...
  // if(pool === null){
  //   log.info('[MasterChef] Log Pool Addition over pid and poolinfo length {} {} {} {}', [
  //     event.params.pid.toString(),
  //     event.params.allocPoint.toString(),
  //     event.params.lpToken.toHex(),
  //   ])
...
export function deposit(event: Deposit): void {
  ...
  if (event.block.number.gt(MASTER_CHEF_START_BLOCK) && user.amount.gt(BIG_INT_ZERO)) {
    const pending = user.amount
      .toBigDecimal()
      .times(pool.accRewardPerShare .toBigDecimal())
